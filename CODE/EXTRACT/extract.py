...
# --- KONEKSI FILE (DATA PIHPS) ---
PIHPS_FILE = r"C:\Users\anggi\Documents\KULIAH\TINGKAT 3\SEMESTER 6\TPD\PROJECT UTS\UTS-TPD-KELOMPOK-1\DATA\PIHPS\final_data.xlsx"
print("[SUCCESS] Modul dan Konfigurasi Koneksi Berhasil Dimuat.")


# ## 1. EXTRACT DATA BPS (API & DUMMY)
# Menarik data dari BPS SIMDASI melalui Web API.
...
print("[SUCCESS]Fungsi scraping berhasil didefinisikan.")
print(f"   Komoditas yang akan di-scrape: {[c['name'] for c in COMMODITIES_TO_SCRAPE]}")


# --- Validasi Koneksi API BPS & Keberadaan Variabel --
print("="*60)
print(" CHECK KONEKSI API BPS ")
print("="*60)
    ...
    for tbl_id in table_ids:
        ...
        if temp_data and temp_data.get("status") == "OK":
            ...
            if _find_var_id(main_data_check, keywords):
                print(f"[SUCCESS] {commodity_name.ljust(35)}: OK (Tabel {tbl_id})")
                status_ok = True
                break
    
    if not status_ok:
        print(f"[FAILED] {commodity_name.ljust(35)}: GAGAL")

# SCRAPING API BPS - FULL RUN
...
for comm in COMMODITIES_TO_SCRAPE:
    ...
    if yearly_dfs:
        ...
        print(f"   [SUCCESS] {len(commodity_dfs[comm['col']])} baris terkumpul")
    else:
        print(f"   [FAILED] Tidak ada data berhasil diambil untuk {comm['name']}")
...
print("[SUCCESS] Generate dummy selesai.")

# PUSH KE POSTGRESQL (STAGING DB BPS)
...
print("[SUCCESS] Tabel staging_bps_api_raw dan staging_bps_dummy_raw berhasil dimuat.")
...
for table in ISIKHNAS_TABLES:
    try:
        ...
        print(f"  [SUCCESS] {table} berhasil diekstrak ({len(df_isikhnas)} baris) -> {staging_table_name}")
    except Exception as e:
        print(f"  [FAILED] Gagal ekstrak tabel {table}: {e}")


# ## 3. EXTRACT DATA PIHPS
...
if os.path.exists(PIHPS_FILE):
    try:
        ...
        print(f"  [SUCCESS] Data PIHPS berhasil diekstrak ({len(df_pihps)} baris) -> staging_pihps_raw")
    except Exception as e:
        print(f"  [FAILED] Gagal memuat file Excel PIHPS: {e}")
else:
    print(f"  [FAILED] File PIHPS tidak ditemukan di path: {PIHPS_FILE}")


# ## VALIDASI TABEL DI STAGING DB
...
>>>>>>>> fdd5fc83fcc879245ca75fbb188614ed2525a58c:CODE/EXTRACT/ETL_Extract_Kelompok1.py
